                ...
                u32 count)
{
    ...
    for (u32 bg_idx = 0; bg_idx < count; bg_idx++)
    {
        ...
        {
            if (layout->binds[i].type == BINDING_TYPE_TEXTURE ||
                layout->binds[i].type == BINDING_TYPE_STORAGE_TEXTURE)
                wgpuTextureViewRelease(desc.entries[i].textureView);
        }
...
write_to_texture(wgpu_renderer_t *wgpu, WGPUTexture texture, texture_info_t info)
{
    ...
    WGPUTextureDataLayout src = {0};
    src.offset = 0;
    src.bytesPerRow =  bytes_per_pixel * info.size[0];
    src.rowsPerImage = info.size[1];
    
    LOG("bytesPerRow: %d, texture: %p, info.data: %p", src.bytesPerRow,
        texture, info.data);
    ...
    u64 size = src.bytesPerRow * info.size[1];
                       ...
                       WGPUShaderModule *modules, u32 module_count)
{
    ...
    for (u32 p_idx = 0; p_idx < pipeline_count; p_idx++)
    {
        ...
        ASSERT_LOG(wgpu->compute_pipelines[p_idx], "Failed to create compute pipeline");
    }
}
                      ...
                      shader_asset_t *assets, u32 count)
{
    ...
    for (u32 i = 0; i < count; i++)
    {
        ...
        WGPUShaderModuleWGSLDescriptor wgsl_source = {0};
        wgsl_source.chain.next = NULL;
        wgsl_source.chain.sType = WGPUSType_ShaderModuleWGSLDescriptor;
    ...
    else
    {
        for (u32 i = 0; i < binding.count; i++)
        {
            ...
            /*if (binding.size != stride)
            {
                ...
                wgpuQueueWriteBuffer(wgpu->queue, buffer,
                                     binding.offset + stride*i + binding.size,
                       ...
                       v3 clear_color, u32 clear)
{
    ...
    for (u32 v_idx = 0; v_idx < vertex_buffer_count; v_idx++) {
        ...
        layout = rb->vb_layouts + p_info->vb_layout_ids[v_idx];
        
        u64 size = layout->data.el_size*layout->data.count;
        wgpuRenderPassEncoderSetVertexBuffer(render_pass, v_idx,
                                             *vertex_buffer, 0,
                                             size);
    }
    
    u64 size = get_stack_count(rb->indices)*sizeof(u32);
...
{
    WGPUInstance instance = wgpu->instance;
    WGPUSurface surface = wgpu->surface;
    ...
    wgpuDeviceSetUncapturedErrorCallback(device, uncaptured_error_callback, NULL);
...
internal void
init_wgpu_renderer(renderer_t *rb, wgpu_renderer_t *wgpu, asset_manager_t *am)
{
...
internal void
start_wgpu_frame(wgpu_renderer_t *wgpu)
{
...
internal void
end_wgpu_frame(renderer_t *rb, wgpu_renderer_t *wgpu)
{

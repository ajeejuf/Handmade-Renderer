...
void set_pos_3(void * args) {
    f32 t = *(f32 *) args;
    
    v2 *pos = metaballs.pos;
    
    pos[0].X = (1.0f - t) * from_x[0] + t * to_x[0];
    pos[1].X = (1.0f - t) * from_x[1] + t * to_x[1];
    
    LOG("pos 0: %f   pos 1: %f", pos[0].X, pos[1].X);
    
    update_bind_group_layout(global_rb, pos_bind_id, 0);
}


GET_CALLBACKS(get_callbacks)
{
    callback_info_t *info = malloc(sizeof(*info)*3);
    
    info[0] = get_callback_info("set_range", strlen("set_range"),
                                TYPE_VOID, set_range_3);
    
    info[1] = get_callback_info("set_res", strlen("set_res"),
                                TYPE_VOID, set_res_3);
    
    info[2] = get_callback_info("set_pos", strlen("set_pos"),
                                TYPE_VOID, set_pos_3);
    
    hashmap_create(2, *func_hash);
    
    ASSERT_LOG(!hashmap_put(*func_hash, "set_range", strlen("set_range"), info),
               "Failed to hash %s function.", "set_range");
    
    ASSERT_LOG(!hashmap_put(*func_hash, "set_res", strlen("set_res"), info+1),
               "Failed to hash %s function.", "set_res");
    
    ASSERT_LOG(!hashmap_put(*func_hash, "set_pos", strlen("set_pos"), info+2),
               "Failed to hash %s function.", "set_pos");
...
INIT_APP(init_app)
{
    ...
    pos[0] = HMM_V2(from_x[0], rb->height/ 2.0f);
    pos[1] = HMM_V2(from_x[1], rb->height/2.0f);
    rad[0] = unit;
    rad[1] = unit;
    vel[0] = HMM_V2(0.0, 0.0);
    vel[1] = HMM_V2(0.0, 0.0);
    
    metaballs = create_metaballs(rb, pos, rad, vel, 2, 2, HMM_V3(1.0f, 0.5f, 0.0f), HMM_V3(1.0f, 0.0f, 0.0f),
                                 rb->width, rb->height);
